...
} from '../models/dto/organization.dto';
...
class OrganizationController implements ExpressController {
  ...
  constructor(
    private service: ServiceFactory<OrganizationService>,
      ...
      async (req, res) => {
        try {
          await this.service
            .create(this.em.fork())
            .createOrganization(
              CreateOrganizationDto.deserializeJsonToEntity(req.body)
            );
    ...
    this.router.get(
      ...
      async (req, res) => {
        const organization = await this.service
          .create(this.em.fork())
          .getOrganization(req.params.id);
        if (organization) {
          res
            .status(200)
            .json(OrganizationDto.serializeEntityToJson(organization));
        } else {
    ...
    this.router.put(
      ...
      async (req, res) => {
        const organization = UpdateOrganizationDto.deserializeJsonToEntity(
          req.body
        );
        await this.service
          .create(this.em.fork())
