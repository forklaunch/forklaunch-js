import { sampleSdkClient2 } from '../servers/express-zod';

describe('nestedPaths', () => {
  it('should call nested getTest', async () => {
    const getTest = await sampleSdkClient2.sdk.sample.path.a.b.testFile[
      '2.0.0'
    ]({
      headers: {
  ...
  it('should fetch nested getTest', async () => {
    const getTest = await sampleSdkClient2.fetch('/testpath/test', {
      method: 'GET',
      headers: {
        authorization: 'bb string',
        'x-test': 'test'
  ...
  it('should call nested postTest', async () => {
    const postTest = await sampleSdkClient2.sdk.sample.path.a.b.testSse[
      '2.0.0'
    ]({
      headers: {
        xyz: 'Basic string'
  ...
  it('should fetch nested postTest', async () => {
    const postTest = await sampleSdkClient2.fetch('/testpath/test', {
      method: 'POST',
      body: {
        f: '!',
        m: [1, 2, 3]
  ...
  it('should call nested jsonPatchTest', async () => {
    const jsonPatchTest =
      await sampleSdkClient2.sdk.sample.path.a.b.testJsonPatch({
        body: {
          f: 'ok',
  ...
  it('should fetch nested jsonPatchTest', async () => {
    const jsonPatchTest = await sampleSdkClient2.fetch('/testpath/test', {
      method: 'PATCH',
      body: {
        f: 'ok',
        h: 'b658f7e0-9b8a-4e1f-b6d8-1c0b7d8b3f59'
  ...
  it('should call nested multipartTest', async () => {
    const multipartTest =
      await sampleSdkClient2.sdk.sample.path.a.b.testMultipart({
        headers: {
          'x-test': 'test'
        },
        body: {
