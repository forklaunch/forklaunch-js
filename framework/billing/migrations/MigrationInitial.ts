...
export class MigrationInitial extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `create table "payment_link" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "amount" int not null, "currency" text check ("currency" in ('USD', 'EUR', 'GBP', 'CAD', 'AUD', 'CHF', 'JPY', 'SEK', 'NOK', 'DKK', 'CZK', 'PLN', 'HUF', 'RON', 'BGN', 'RUB', 'INR', 'BRL', 'ZAR', 'MXN', 'CNY', 'HKD', 'TWD', 'SGD', 'THB', 'TRY', 'RSD', 'HRK', 'ISK', 'BAM')) not null, "description" text not null, "metadata" jsonb null, "success_redirect_uri" text not null, "cancel_redirect_uri" text not null, "extra_fields" jsonb null, constraint "payment_link_pkey" primary key ("id"));`
    );
