...
export class Migration20260710162344 extends Migration {
  ...
  override up(): void | Promise<void> {
    ...
    this.addSql(`create table "order" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "retention_anonymized_at" timestamptz null, "customer_id" text null, "status" text not null, "items" jsonb not null, "subtotal_cents" int not null, "tax_cents" int not null, "total_cents" int not null, primary key ("id"));`);
    ...
    this.addSql(`create table "subscription" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "retention_anonymized_at" timestamptz null, "customer_id" text not null, "items" jsonb not null, "interval_days" int not null, "status" text not null, "next_order_at" timestamptz not null, "provider_sub_ref" text null, primary key ("id"));`);
    this.addSql(`alter table "subscription" add constraint "subscription_status_check" check ("status" in ('active', 'paused', 'canceled'));`);
