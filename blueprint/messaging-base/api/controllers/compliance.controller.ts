...
export const eraseUserData = handlers.delete(
  ...
  '/erase/:userId',
  {
    ...
    access: 'protected',
    summary:
      'Erases all PII/PHI/PCI data for a user from messaging entities (GDPR Art. 17)',
    auth: {
      jwt: {
...
export const exportUserData = handlers.get(
  ...
  '/export/:userId',
  {
    ...
    access: 'protected',
    summary:
      'Exports all PII/PHI/PCI data for a user from messaging entities (GDPR Art. 20)',
    auth: {
      jwt: {
        jwksPublicKeyUrl: JWKS_PUBLIC_KEY_URL
      },
