...
- **`cli/src/core/manifest/service.rs`** + `init/module.rs` + `init/application.rs` + `init/service.rs` — new `is_cac` / `is_cac_configured` manifest flags, threaded through the same places `is_messaging`/`is_iam` are today.
