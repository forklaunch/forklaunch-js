...
8. **Does `cac-base` need its own env vars / Redis dependency?** §3's CLI checklist flags this as conditional — two independent reasons have surfaced since: §7's caching design (Redis-backed scrubbing lookups) and, as of PR 5, caching the real `surfacePermissions`/`surfaceRoles` IAM calls (`surfacing.ts`) so every request doesn't re-hit IAM. Neither is wired in yet. (No longer tied to a Stedi integration — that's out of scope, §8.)
