...
- **How it ships:** PR 1 (CLI module registration + blueprint skeleton) is merged; PR 2 (all §4 entities, the ICD-10-CM/HCPCS loaders, and the `refresh-code-sets.ts` ETL shape) is implemented; PR 3 (claim engine, three-layer scrubbing, and the real-CPT extension point, all mock-code-tested) is implemented. With claim submission now out of scope, the original six-PR plan collapses to those three PRs plus a documentation/hardening pass — see §14 for the current breakdown.
...
8. **Does `cac-base` need its own env vars / Redis dependency?** §3's CLI checklist flags this as conditional — resolve once §7's caching design (Redis-backed scrubbing lookups) is scoped. (No longer tied to a Stedi integration — that's out of scope, §8.)
...
12. **Does the module still want a denial worklist API, now that PR 4 is removed?** The one piece of old PR 4 that didn't depend on Stedi — listing/viewing/resolving the `Denial` rows PR 3's scrubbing engine already creates — has no clearinghouse dependency and could still be built. It just isn't part of any current phase's scope anymore since the phase it belonged to was removed wholesale. Worth a deliberate yes/no rather than falling out of scope by accident.
...
10. Decide whether a standalone denial-worklist API (§12 item 12) is worth building now that it's no longer bundled with PR 4's (removed) eligibility/remittance scope.
...
| ~~PR 4~~ | ~~Phase 3~~ | ~~Eligibility & remittance — 270/271, 835, 277CA/999, denial worklist~~ **Removed (2026-08-31) — out of scope, not deferred.** `cac-base` never submits claims or parses remittances on an adopter's behalf; that's their own downstream engineering. The one piece with no clearinghouse dependency — a denial worklist API over the `Denial` rows PR 3 already creates — could still be built if wanted, but isn't part of this phase's original scope anymore either; raise it as new scope if needed |
