...
RUST_BACKTRACE=1 cargo run --release init application change-worker-test-node-application -p . -d postgresql -f prettier -l eslint -v zod -F express -r bun -t vitest -m billing-base -m iam-base -D "Test worker" -A "Rohin Bhargava" -L 'AGPL-3.0'
