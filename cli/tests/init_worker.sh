...
RUST_BACKTRACE=1 cargo run --release init application worker-test-node-application -p ./worker-test-node-application -o source -d postgresql -f prettier -l eslint -v zod -F express -r node -t vitest -m billing-base -m iam-base -D "Test worker" -A "Rohin Bhargava" -L 'AGPL-3.0'
...
RUST_BACKTRACE=1 cargo run --release init worker wrk-test-redis -t redis -p worker-test-node-application/src/modules -D "Test worker"
...
RUST_BACKTRACE=1 cargo run --release init application worker-test-bun-application -p ./worker-test-bun-application -o source -d postgresql -f biome -l oxlint -v zod -F express -r bun -t vitest -m billing-base -m iam-base -D "Test worker" -A "Rohin Bhargava" -L "MIT"
...
RUST_BACKTRACE=1 cargo run --release init worker wrk-test-redis -t redis -p worker-test-bun-application/src/modules -D "Test worker"
