...
RUST_BACKTRACE=1 cargo run --release init application worker-test-node-application -p ./worker-test-node-application -o src -d postgresql -f prettier -l eslint -v zod -F express -r node -t vitest -m billing-base -m iam-base -D "Test worker" -A "Rohin Bhargava" -L 'AGPL-3.0'
...
RUST_BACKTRACE=1 cargo run --release init application worker-test-bun-application -p ./worker-test-bun-application -o src -d postgresql -f biome -l oxlint -v zod -F express -r bun -t vitest -m billing-base -m iam-base -D "Test worker" -A "Rohin Bhargava" -L "MIT"
