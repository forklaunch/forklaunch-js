...
RUST_BACKTRACE=1 cargo run --release init application module-test-src-path -p ./module-test-src-path -o src -d postgresql -f prettier -l eslint -v zod -F express -r node -t vitest -D "Test service" -A "ForkLaunch" -L 'AGPL-3.0'
