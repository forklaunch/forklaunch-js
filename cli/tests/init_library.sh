...
RUST_BACKTRACE=1 cargo run init application library-test-node-application -d postgresql -f prettier -l eslint -v zod -F express -r node -t vitest -s billing -s iam -D "Test library" -A "Rohin Bhargava" -L 'AGPL-3.0'
...
RUST_BACKTRACE=1 cargo run init application library-test-bun-application -d postgresql -f biome -l oxlint -v zod -F express -r bun -t vitest -s billing -s iam -D "Test library" -A "Rohin Bhargava" -L 'AGPL-3.0'
