...
RUST_BACKTRACE=1 cargo run --release init application router-test-bun-application -p ./router-test-bun-application -o source -d postgresql -f biome -l oxlint -v zod -F express -r bun -t vitest -m billing-base -m iam-base -D "Test service" -A "Rohin Bhargava" -L "MIT"
