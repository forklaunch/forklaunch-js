...
RUST_BACKTRACE=1 cargo run --release init application router-test-node-application -p . -o source -d postgresql -f prettier -l eslint -v zod -F express -r node -t vitest -m billing-base -m iam-base -D "Test service" -A "Rohin Bhargava" -L 'AGPL-3.0'
...
RUST_BACKTRACE=1 cargo run --release init application router-test-bun-application -p . -o source -d postgresql -f biome -l oxlint -v zod -F express -r bun -t vitest -m billing-base -m iam-base -D "Test service" -A "Rohin Bhargava" -L "MIT"
