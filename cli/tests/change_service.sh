...
RUST_BACKTRACE=1 cargo run --release init application change-service-test-node-application -d postgresql -f prettier -l eslint -v zod -F express -r bun -t vitest -s billing -s iam -D "Test service" -A "Rohin Bhargava" -L 'AGPL-3.0'
