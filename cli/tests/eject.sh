...
RUST_BACKTRACE=1 cargo run --release init application service-test-node-application -d postgresql -f prettier -l eslint -v zod -F express -r node -t vitest -s billing-base -s iam-base -D "Test service" -A "Rohin Bhargava" -L 'AGPL-3.0'
