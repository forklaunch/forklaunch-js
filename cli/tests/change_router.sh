...
RUST_BACKTRACE=1 cargo run --release init application change-router-test-node-application -p ./change-router-test-node-application -o src -d postgresql -f prettier -l eslint -v zod -F express -r bun -t vitest -m billing-base -m iam-base -D "Test router" -A "Rohin Bhargava" -L 'AGPL-3.0'
