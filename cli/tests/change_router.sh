...
RUST_BACKTRACE=1 cargo run init application change-router-test-node-application -d postgresql -f prettier -l eslint -v zod -F express -r bun -t vitest -s billing -s iam -D "Test router" -A "Rohin Bhargava" -L 'AGPL-3.0'
