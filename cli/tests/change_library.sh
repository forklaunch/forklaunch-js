...
RUST_BACKTRACE=1 cargo run --release init application change-library-test-node-application -p . -d postgresql -f prettier -l eslint -v zod -F hyper-express -r bun -t vitest -m billing-base -m iam-base -D "Test service" -A "Rohin Bhargava" -L 'AGPL-3.0'
