...
RUST_BACKTRACE=1 $FL init service svc-test -d postgresql -D "Test service"
...
RUST_BACKTRACE=1 $FL init service svc-test -d postgresql -p . -D "Test service"
RUST_BACKTRACE=1 $FL delete service svc-test -p . -c
