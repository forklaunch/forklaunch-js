...
RUST_BACKTRACE=1 cargo run --release init library lib-dummy -p ./sync-test-node-application -D "Test library"
RUST_BACKTRACE=1 cargo run --release init service svc-dummy -d postgresql -p ./sync-test-node-application/src/modules -D "Test service"
RUST_BACKTRACE=1 cargo run --release init worker wrk-dummy -t database -d postgresql -p ./sync-test-node-application/src/modules -D "Test worker"
RUST_BACKTRACE=1 cargo run --release init router rtr-dummy -p ./sync-test-node-application/src/modules/billing
RUST_BACKTRACE=1 cargo run --release init router rtr-dummy-two -p ./sync-test-node-application/src/modules/iam
...
RUST_BACKTRACE=1 cargo run --release sync all -p .